...
def prepare_splits(dataset):
    ...
    elif split_mode.startswith('lopo-'):
        setup_lopo_split(dataset)
    else:
        raise ValueError(f"Unknown split mode: {split_mode}")


def setup_lopo_split(dataset):
    """Generate Leave-One-Person-Out splits.

    Identify samples belonging to the test and validation subjects and
    assign them to the corresponding splits. All other samples are assigned
    to the training split.
    """
    test_subj = cfg.dataset.test_subj
    val_subj = cfg.dataset.val_subj
    
    if test_subj == 'None':
        # Default to a specific subject if none provided (e.g., first one found)
        # but better to error out to ensure user knows what's happening
        raise ValueError("LOPO split requires cfg.dataset.test_subj to be set.")
        
    train_index = []
    val_index = []
    test_index = []
    
    # We expect Data objects to have a 'subject' attribute
    for i, data in enumerate(dataset):
        subject = getattr(data, 'subject', 'unknown')
        if subject == test_subj:
            test_index.append(i)
        elif subject == val_subj:
            val_index.append(i)
        else:
            train_index.append(i)
            
    # Fallback: if val_index is empty, take a portion of train_index for validation
    if len(val_index) == 0 and len(train_index) > 0:
        # Take 10% of training for validation if no specific val subject is provided
        val_size = max(1, len(train_index) // 10)
        val_index = train_index[-val_size:]
        train_index = train_index[:-val_size]
        logging.info(f"[*] LOPO: No val_subj provided. Using last {val_size} training samples for validation.")

    logging.info(f"[*] LOPO Split: Test Subj={test_subj}, Val Subj={val_subj}")
    logging.info(f"    Train: {len(train_index)} | Val: {len(val_index)} | Test: {len(test_index)}")
