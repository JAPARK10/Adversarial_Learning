...
def setup_random_split(dataset):
    ...
    set_dataset_splits(dataset, [train_index.tolist(), val_index.tolist(), test_index.tolist()])
...
def setup_lopo_split(dataset):
    ...
    # We expect dataset.data.participant to be a tensor of participant IDs (0-15)
    if not hasattr(dataset.data, 'participant'):
        raise ValueError("Dataset missing 'participant' attribute. Please re-process the dataset.")
    
    p_ids = dataset.data.participant
    unique_ps = sorted(p_ids.unique().tolist())
    num_p = len(unique_ps)
    
    if split_index >= num_p:
        raise ValueError(f"Split index {split_index} out of range for {num_p} participants.")
        
    test_p = unique_ps[split_index]
    val_p = unique_ps[(split_index + 1) % num_p]
    
    # Identify indices
    test_idx = torch.where(p_ids == test_p)[0].tolist()
    val_idx = torch.where(p_ids == val_p)[0].tolist()
    train_idx = torch.where((p_ids != test_p) & (p_ids != val_p))[0].tolist()
    
    logging.info(f"[*] LOPO Fold {split_index}: Test P {test_p}, Val P {val_p}")
