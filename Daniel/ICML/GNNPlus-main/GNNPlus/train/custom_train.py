...
def train_epoch(logger, loader, model, optimizer, scheduler, batch_accumulation):
    ...
    for iter, batch in enumerate(loader):
        ...
        if cfg.adv.use:
            pred, true, pred_user, true_user = model(batch)
        else:
        ...
        if cfg.dataset.name == 'ogbg-code2':
            ...
            loss_g, pred_score = compute_loss(pred, true)
            if cfg.adv.use:
                loss_u = F.cross_entropy(pred_user, true_user.view(-1))
                loss = loss_g + loss_u
            else:
                loss = loss_g
        ...
        if cfg.adv.use:
            ...
            # Calculate user accuracy for training batch
            user_pred = pred_user.argmax(dim=1)
            user_acc = (user_pred == true_user.view(-1)).float().mean().item()
            extra_stats['user_acc'] = user_acc
...
def custom_train(loggers, loaders, model, optimizer, scheduler):
    ...
    for cur_epoch in range(start_epoch, cfg.optim.max_epoch):
        ...
        if is_eval_epoch(cur_epoch):
            ...
            if best_epoch == cur_epoch:                    
                ...
                conf_dir = os.path.join(cfg.out_dir, 'Conf')
                ...
                for cm_batch in loaders[2]: # Test loader
                    ...
                    with torch.no_grad():
                        outputs = model(cm_batch)
                        cm_pred, cm_true = outputs[0], outputs[1]
    ...
    with open(f'{cfg.out_dir}/{cfg.dataset.name}_result.txt','a') as f:
